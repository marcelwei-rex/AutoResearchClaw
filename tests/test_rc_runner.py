...
@pytest.mark.parametrize(
    ...
    [("pivot", Stage.HYPOTHESIS_GEN), ("refine", Stage.ITERATIVE_REFINE)],
)
def test_stage15_rollback_recursion_honors_outer_to_stage_paper_outline(
    monkeypatch: pytest.MonkeyPatch,
    run_dir: Path,
    rc_config: RCConfig,
    adapters: AdapterBundle,
    ...
    # to_stage=PAPER_OUTLINE: the Stage-15 rollback recursion may reach
    # Stage 16 (boundary is inclusive) but MUST NOT overshoot to Stage 17+.
    # RED before the runner forwards to_stage into the recursive call.
    seen: list[Stage] = []
    decision_count = 0
    ...
    def mock_execute_stage(stage: Stage, **kwargs) -> StageResult:
        ...
        if stage == Stage.RESEARCH_DECISION and decision_count == 0:
            decision_count += 1
            return StageResult(
                stage=stage,
                status=StageStatus.DONE,
    ...
    results = rc_runner.execute_pipeline(
        run_dir=run_dir,
        run_id=f"run-{decision}-outline-boundary",
        config=rc_config,
        adapters=adapters,
        to_stage=Stage.PAPER_OUTLINE,
    )

    assert rollback_target in seen  # rollback recursion actually ran
    assert Stage.PAPER_OUTLINE in seen  # boundary is inclusive
    overshot = [s for s in seen if int(s) > int(Stage.PAPER_OUTLINE)]
    assert overshot == [], f"rollback recursion overshot to_stage: {overshot}"
    assert not [r for r in results if int(r.stage) > int(Stage.PAPER_OUTLINE)]


def test_stage15_decision_at_research_decision_boundary_skips_recursion(
    monkeypatch: pytest.MonkeyPatch,
    run_dir: Path,
    rc_config: RCConfig,
    adapters: AdapterBundle,
) -> None:
    # to_stage=RESEARCH_DECISION: existing safe path — no pivot recursion and
    # Stage 16 never reached. Regression guard for the to_stage-forwarding fix.
    seen: list[Stage] = []
    pivoted = False
    ...
    def mock_execute_stage(stage: Stage, **kwargs) -> StageResult:
        ...
        seen.append(stage)
        nonlocal pivoted
        if stage == Stage.RESEARCH_DECISION and not pivoted:
            pivoted = True
            return _pivot_result(stage)
        return _done(stage)
    ...
    results = rc_runner.execute_pipeline(
        run_dir=run_dir,
        run_id="run-decision-boundary",
        config=rc_config,
        adapters=adapters,
        to_stage=Stage.RESEARCH_DECISION,
    )

    assert seen.count(Stage.HYPOTHESIS_GEN) == 1  # no rollback recursion
    assert Stage.PAPER_OUTLINE not in seen  # stops at Stage 15
    assert all(int(r.stage) <= int(Stage.RESEARCH_DECISION) for r in results)
